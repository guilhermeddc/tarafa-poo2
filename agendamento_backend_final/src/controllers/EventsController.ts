...
class EventsController {
  async index(request: Request, response: Response) {
    // Adiciondo page e limit por query
    ...
    const events = await knex("rooms_events")
      ...
      .orderBy("date_time")
      // Adicionado offset = page e limit
      .paginate({ perPage: 2, currentPage });

    const serializedItens = events.data.map((event) => {
      const buildingsArray = event.building.split(",");
      ...
      return {
        ...
        date_time: format(
          event.date_time,
          "'Data: 'dd'/'MM'/'yyyy 'Horário:' HH':'mm"
        ),
        responsible: event.responsible,
      };
    });

    events.data = serializedItens;
  ...
  async eventsOfDay(request: Request, response: Response) {
    ...
    const total = await knex("rooms_events")
      .count()
      .innerJoin("events", "events.id_event", "=", "rooms_events.id_event")
      .innerJoin("rooms", "rooms.id_room", "=", "rooms_events.id_room")
    ...
    for (var i = 0; i < rooms.length; i++) {
      ...
      if (findEventInSameDate) {
        ...
        return response.send(
          "There is an event already booked in the same local for this date and time!"
        );
      }
    }
